...
const galleryPrev = document.querySelector('.gallery__control[data-direction="prev"]');
const galleryNext = document.querySelector('.gallery__control[data-direction="next"]');
...
function getGalleryMaxPage() {
  ...
  return Math.max(0, Math.ceil(galleryItems.length / getGalleryItemsPerPage()) - 1);
}
...
function updateGallery() {
  ...
  const targetIndex = Math.min(galleryItems.length - 1, galleryPage * getGalleryItemsPerPage());
...
function updateScoreboard() {
  ...
  const completedCount = Object.values(missionState).filter((m) => m.completed).length;
  if (progressFill) {
  ...
  if (progressBar) {
    ...
    progressBar.setAttribute('aria-valuetext', `${completedCount} з 3 місій завершено`);
  }
}
...
function showMission(key) {
  ...
  if (key === 'intro' || key === 'final') {
    missionCards.forEach((card) => card.classList.remove('mission-card--active'));
  } else {
    missionCards.forEach((card) => {
      card.classList.toggle('mission-card--active', card.dataset.mission === key);
    });
...
function resetHistoryMission() {
  ...
  shuffled.forEach((event) => {
    ...
    button.addEventListener('click', () => handleHistoryChoice(card, event));
...
function handleHistoryChoice(card, event) {
  ...
  if (event.id === expectedEvent.id) {
    ...
    if (historyState.currentIndex === historyEvents.length) {
      historyFeedback.textContent = 'Хронологію відновлено! Ви можете завершити місію.';
      historyComplete.disabled = false;
    }
  } else {
    card.classList.add('is-incorrect');
    historyState.errors += 1;
    historyFeedback.textContent = 'Спробуйте іншу подію. Зверніть увагу на підказки у описах.';
...
function startExpedition() {
  ...
  showMission('intro');
  document.getElementById('missionArea').scrollIntoView({ behavior: 'smooth' });
  randomFact();
}
...
galleryItems.forEach((item) => {
  ...
  item.addEventListener('click', () => {
    openGalleryModal(img.currentSrc || img.src, img.alt || '', caption ? caption.textContent : '');
  });
});
